...
document.getElementById('lb-btn').addEventListener('click', function() {
    ...
    clearError();
    if ([Pt,Ltx,Gt,f,d,Lmisc,Gr,Lrx,NF,BW,SNR_req].some(isNaN)||f<=0||d<=0||BW<=0) {
        showError('Please fill in all fields with valid values.'); return;
    }
    var lam=3e8/f;
    ...
    var box=document.getElementById('lb-out');
    box.innerHTML='<table style="font-family:monospace;font-size:13px;border-collapse:collapse;">'
        +row('EIRP',EIRP,'dBm')+row('Free-space path loss',FSPL,'dB')+row('Additional losses',Lmisc,'dB')
        ...
        +'<tr><td style="padding:2px 12px 2px 0;"><b>Link margin</b></td><td style="text-align:right;padding:2px 6px;color:'+color+';"><b>'+margin.toFixed(2)+'</b></td><td>dB</td></tr>'
        +'</table>';
});
function row(lbl,val,unit){return '<tr><td style="padding:2px 12px 2px 0;">'+lbl+'</td><td style="text-align:right;padding:2px 6px;">'+val.toFixed(2)+'</td><td>'+unit+'</td></tr>';}
