...
document.getElementById('es-btn').addEventListener('click', es_calc);

function es_calc() {
  clearError();
  var raw    = parseFloat(document.getElementById('es-val').value.trim());
  ...
  var nearest = findNearest(target, series);
  showResults(nearest, target, series, info.base);
}

function findNearest(target, series) {
  var table = E_SERIES[series];
  var log10 = Math.log(target) / Math.LN10;
  var decade = Math.floor(log10);
  var candidates = [];

  for (var d = decade - 1; d <= decade + 1; d++) {
    var mul = Math.pow(10, d);
    table.forEach(function(v) { candidates.push(v * mul); });
  }
  ...
  candidates.forEach(function(v) {
    var key = v.toPrecision(4);
    if (!seen[key] && out.length < 5) { seen[key] = true; out.push(v); }
  });
  return out;
}

function showResults(vals, target, series, baseUnit) {
  var container = document.getElementById('es-results');
  container.innerHTML = '';
  container.style.display = 'grid';

  var card = document.createElement('div');
  card.className = 'res-card';
  card.innerHTML = '<h4>' + series + ' — Nearest 5 Values</h4>' +
    '<div class="res-row" style="font-size:11px;color:#888;margin-bottom:4px;">' +
    '<span class="res-lbl">Standard value</span>' +
    '<span class="res-val">Error from target</span></div>';

  vals.forEach(function(v, i) {
    var err = (v - target) / target * 100;
    var errStr = (err >= 0 ? '+' : '') + err.toFixed(2) + '%';
    var label = engFmt(v, baseUnit);
    var hi = (i === 0) ? ' style="font-weight:700;color:#AA77FF;"' : '';
    card.innerHTML +=
      '<div class="res-row">' +
      '<span class="res-lbl"' + hi + '>' + label + '</span>' +
      '<span class="res-val"' + hi + '>' + errStr + '</span></div>';
  });

  container.appendChild(card);
  if (window.drawDiagram) window.drawDiagram(series, vals.length);
}
