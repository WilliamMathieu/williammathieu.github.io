...
document.getElementById('ms-btn1').addEventListener('click', function() {
    ...
    document.getElementById('ms-err1').textContent='';
    if(isNaN(h)||isNaN(w)||isNaN(er)||h<=0||w<=0||er<=0){document.getElementById('ms-err1').textContent='Enter valid positive values for h, w, and \u03b5r.';return;}
    var wEff=w; if(t>0&&t<h) wEff=w+(t/Math.PI)*(1+Math.log(2*h/t));
...
document.getElementById('ms-btn2').addEventListener('click', function() {
    ...
    document.getElementById('ms-err2').textContent='';
    if(isNaN(Z0)||isNaN(h)||isNaN(er)||Z0<=0||h<=0||er<=0){document.getElementById('ms-err2').textContent='Enter valid positive values for Z\u2080, h, and \u03b5r.';return;}
