...
\item You double the number of turns on a fixed core. By what factor does the inductance change?
  \begin{answer}\textbf{Answer.} $\times 4$, since $L \propto N^{2}$.\end{answer}
\item Why is laminated silicon-steel unsuitable for an inductor at $100\ \mathrm{MHz}$?
  \begin{answer}\textbf{Answer.} Eddy-current and hysteresis losses grow rapidly with frequency; a high-resistivity ferrite is used instead.\end{answer}
...
At that frequency is the impedance mainly reactive or resistive, and why?
  \begin{answer}\textbf{Answer.} Mainly resistive --- the bead is chosen to operate where the ferrite is lossy, so it \emph{absorbs} the unwanted RF rather than storing it.\end{answer}
\end{exercises}
